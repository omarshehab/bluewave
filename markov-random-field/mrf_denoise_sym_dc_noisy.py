...
def main():
	...
	im = Image.open(name + '.bmp')
        org_im_shape = (im.size)
        logger.info("Shape of the original image: " + str(org_im_shape))
        ...
        logger.info("Stitch up the tiles...")
        new_im = Image.new('L', org_im_shape)

        for i in xrange(0, org_im_shape[0], 32):
           print "Title row: " + str(i)
           logger.info("Tile row: " + str(i))
           for j in xrange(0, org_im_shape[1], 32):
              print "Title row: " + str(i)
              print "Title column: " + str(j)
              logger.info("Tile column: " + str(j))
              tile = im.crop((i, j, i + 32, j + 32))
	      im_tile = np.array(tile)

              logger.info("Converting into binary...")
	      im_tile = where (im_tile>100,1,0) #convert to binary image
              logger.info("Segmenting the binary image: ")
              out_bin = MRF_denoise_sym(im_tile)
              logger.info("Saving the segmented binary image...")
              out_bin_im = Image.fromarray(np.uint8(cm.gist_earth(out_bin)*255))
              new_im.paste(out_bin_im, (i,j))

        new_im.save(name + '-dc-segmented.bmp')
